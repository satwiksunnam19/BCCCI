...
class RobustHurricaneDataPipeline:
    ...
    def authenticate_earthaccess_with_timeout(self, timeout_seconds=30):
        """Authenticate with NASA Earthdata with timeout protection"""
        def auth_worker():
            try:
                # Try different authentication methods
                auth_result = earthaccess.login(strategy="interactive")
                if auth_result:
...
def main():
    ...
    with st.expander("NASA Earthdata Setup Instructions"):
        st.markdown("""
        **To use NASA Earthaccess, you need:**
        
        1. **NASA Earthdata Account** (Free):
           - Register at: https://urs.earthdata.nasa.gov/
           - Approve NASA applications for data access
        ...
        3. **Required Permissions:**
           - HLS (Harmonized Landsat Sentinel) data access
           - EARTHDATA_CLOUD approval
        
        4. **Troubleshooting:**
           - Ensure firewall allows NASA server connections
           - Check credentials are correct
           - Try manual authentication first outside the app
        """)
