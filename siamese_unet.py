...
class ProductionFloodDataset(Dataset):
        ...
        for pair in tqdm(self.hurricane_pairs, desc="Creating Patches"):
            ...
            if len(flood_y) > 0:
                for _ in range(target_flood):
                    ...
                    top = max(0, cy - self.patch_size // 2); left = max(0, cx - self.patch_size // 2)
                    if top + self.patch_size > min_h or left + self.patch_size > min_w: continue
                    patches.append({
                        'pre': pre_composite[:, top:top+self.patch_size, left:left+self.patch_size],
                        'post': post_composite[:, top:top+self.patch_size, left:left+self.patch_size],
                        'labels': full_labels[top:top+self.patch_size, left:left+self.patch_size]
                    })
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'labels': torch.FloatTensor(patch['labels'])
        }

# --- 3. LOSS FUNCTION (FORTIFIED) ---
class BalancedFocalLoss(nn.Module):
    def __init__(self, alpha=0.5, gamma=2.0, pos_weight_factor=5.0):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
    ...
    def forward(self, logits, targets):
        pos_weight = torch.tensor([self.pos_weight_factor], device=logits.device)
...
class ProductionTrainer:
    def __init__(self, model, train_loader, val_loader, device='mps'):
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'min', patience=5, factor=0.5, verbose=True)
    ...
    def train_epoch(self):
        ...
        for batch in tqdm(self.train_loader, desc="Training"):
            pre, post, labels = batch['pre'].to(self.device), batch['post'].to(self.device), batch['labels'].to(self.device)
    ...
    def train(self, epochs=15):
        ...
        for epoch in range(epochs):
            ...
            for key, value in metrics.items(): self.history[f'val_{key}'].append(value)
...
# --- 5. NEW INFERENCE FUNCTION --- 
def generate_flood_map(model_path, pre_image_path, post_image_path, device='mps'):
    print("\n--- Generating Flood Map from Trained Model ---")
    # Load model architecture and weights
    model = ProductionSiameseUNet().to(device)
    ...
    pre_tensor = torch.FloatTensor(pre_img).unsqueeze(0).to(device)
    post_tensor = torch.FloatTensor(post_img).unsqueeze(0).to(device)
    
    with torch.no_grad():
        logits = model(pre_tensor, post_tensor)
        probabilities = torch.sigmoid(logits).squeeze(0)
    
    flood_map_np = probabilities.cpu().numpy()
    print(f"✅ Flood map generated with shape: {flood_map_np.shape}")
    return flood_map_np

# --- 6. EXECUTION LOGIC (WITH ANALYSIS) ---
def run_production_training():
    ...
    if not metadata_file.exists():
        print("Metadata file not found!"); return
        
    with open(metadata_file, 'r') as f: metadata = json.load(f)
    
    all_files = [{'hurricane': hd.get('hurricane', 'Unknown'), 'file_path': p_info.get('file_path', ''), 'type': 'pre' if 'pre' in p_name else 'post'} for hd in metadata.get('processed_data', []) for p_name, p_info in hd.get('periods', {}).items() if p_info.get('file_path', '') and Path(p_info.get('file_path', '')).exists()]

    pre_files = [f for f in all_files if f['type'] == 'pre']; post_files = [f for f in all_files if f['type'] == 'post']
    if not pre_files or not post_files: print("Error: Not enough pre/post files."); return

    pre_split_idx = max(1, int(0.8 * len(pre_files)))
    train_pre, val_pre = pre_files[:pre_split_idx], pre_files[pre_split_idx:]
    if not val_pre and len(pre_files) > 1: val_pre, train_pre = [pre_files[-1]], pre_files[:-1]

    post_split_idx = max(1, int(0.8 * len(post_files)))
    train_post, val_post = post_files[:post_split_idx], post_files[post_split_idx:]
    if not val_post and len(post_files) > 1: val_post, train_post = [post_files[-1]], post_files[:-1]

    train_files, val_files = train_pre + train_post, val_pre + val_post
    ...
    if len(train_dataset) == 0 or len(val_dataset) == 0: print("Failed to generate patches."); return
    ...
    plt.subplot(1, 2, 1); plt.plot(history['train_loss'], label='Train Loss'); plt.plot(history['val_loss'], label='Val Loss'); plt.legend(); plt.title('Loss'); plt.grid(True, alpha=0.3)
    plt.subplot(1, 2, 2); plt.plot(history['val_f1'], label='F1'); plt.plot(history['val_precision'], label='Precision'); plt.plot(history['val_recall'], label='Recall'); plt.legend(); plt.title('Metrics'); plt.ylim(0, 1); plt.grid(True, alpha=0.3)
    plt.tight_layout(); plt.savefig('training_results.png'); plt.show()
    
    # --- NEW: RUN ANALYSIS AFTER TRAINING ---
    # Use the first validation file pair for the analysis
    val_pre_path = val_files[0]['file_path']
    val_post_path = val_files[1]['file_path']
    bbox = [-82.8, 25.8, -81.2, 27.5] # Example BBox for Florida

    # Generate the flood map using the best model
